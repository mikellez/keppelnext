...
import { CMMSFaultTypes, CMMSPlant, CMMSUser } from '../../types/common/interfaces';
...
const WorkflowNew = ({ 
    ...
    const timelineElements: TimelineElement[] = [
        ...
        },
        {
            ...
            fieldContent: [
                { 
                    type: 'select', label: 'Please select plant location', required: true, options: plants.map(item=>({label: item.plant_name, value: item.plant_id}))
                }
            ]
        },
        {
            ...
            fieldContent: [
                { 
                    type: 'select', label: 'Please select type of fault', required: true, options: faultsTypes.map(item=>({label: item.fault_type, value: item.fault_id})) 
                }
            ]
        },
        {
            ...
            fieldContent: [
                { 
                    type: 'select', label: 'Please select action', required: true, options: [ 
        ...
        },
        {
            ...
            fieldContent: [
                { 
                    type: 'select', label: 'Assign to', required: true, options: assignTo.map(item=>({label: item.name +' | '+item.email, value: item.id}))
                }
            ]
        },
        {
    ...
    const handleSelectChange: React.ChangeEventHandler<HTMLSelectElement> = ( e: { currentTarget: {id: any, value: number, name: string}} ) => {
    ...
    const handleTextChange: React.ChangeEventHandler<HTMLInputElement> = ( e: { currentTarget: {id: any, value: number, name: string}}) => {
        const { id, value, name } = e.currentTarget;

        setTimeline(timeline.map((element, index) => {
            if (element.id === id) {
                element.value = value;
            }
            return element;
    ...
    const validate = () => {
        ...
        for (const element of timeline) {
            if (!element?.hidden && element.fieldContent[0].required && (element.value === "" || element.value === 0)) {
                                        ...
                                        <Card title="Rule Details" style={{ width: 300 }}>
                                            <Timeline
                                                items={
                                                    ...
                                                    .map((element, index) => ({
                                                        children: ( 
                                                            <div style={{ cursor: 'pointer'}} key={element.id} id={element.id} className={index === active ? styles.active : ''} onClick={() => setActive(index)} >
                                                                {element.content} &nbsp;
                                                                {
                                        ...
                                        .map((element, index) => {
                                            if (index === active) {
                                                return (
                                                                        ...
                                                                        return (
                                                                            <div className="form-group" key={field.label}>
                                                                                <label className="form-label">
                                                                                    {field.label} {field.required && <RequiredIcon />}
                                                                                </label>
                                                                                <select id={element.id} name={element.name} className="form-control" onChange={(e) => handleSelectChange(e)} value={element.value}>
                                                                                    <option value="">-- Select Type --</option>
                                                                                    {
                                                                                        field.options.map((option) => {
                                                                                            return (
                                                                                                <option value={option.value} key={option.value} data-depend={option?.depend}>{option.label}</option>
                                                                                            )
                                                                                        })
                                                                                    }
                                                                                </select>
                                                                    ...
                                                                    case 'text':
                                                                        return (
                                                                            <div className="form-group" key={field.label}>
                                                                                ...
                                                                                <input id={element.id} name={element.name} type="text" placeholder="Enter Email Address" className="form-control" onChange={(e) => handleTextChange(e)}/>
                                                                            </div>
                                                                        )
                                                                        break;
                                                                }
                                                    ...
                                                    {active > 0 && <button className="btn btn-primary btn-sm" onClick={() => setActive(--index)}>Back</button>}
                                                    &nbsp; &nbsp;
                                                    {active < timeline.filter(element=>!element.hidden).length - 1 && <button className="btn btn-primary btn-sm" onClick={() => setActive(++index)}>Next</button>}
                                                    {active == timeline.filter(element=>!element.hidden).length - 1 && <button className="btn btn-primary btn-sm flex" onClick={handleSubmit}>Submit Workflow</button>}
                                                    </>
                                                )
                                            }
                                        })
