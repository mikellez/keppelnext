...
async function getAssets(plant_id) {
  return await instance
    .get("/api/request/getAssets/" + plant_id)
    .then((response) => {
      return response.data;
    })
    .catch((e) => {
...
function processQR(arrSelectedAssets) {
  ...
  const plant_name =
    document.getElementById("plantLocation").options[
      document.getElementById("plantLocation").selectedIndex
    ].text; // get plant name
  const qrElement = document.getElementById("qrcode");
  const arrPromises = [];
  const qrDoc = document.implementation.createHTMLDocument("QR Code");
  qrDoc.documentElement.innerHTML =
    "<head><style>#container{padding: 2em 2em 2em 2em;display:flex;flex-wrap:wrap;justify-content: space-around; font-size:22px;}#container div{padding: 0 1em 2.5em 1em;flex: 0 0 20%;}#container div img{margin: 0;position: relative;left: 50%;-ms-transform: translateX(-50%);transform: translate(-50%);image-rendering: crisp-edges;padding-bottom: 0.5em;}#container div div{text-align: center;}</style></head><body><div id='container'></div></body>";
  const qrDiv = qrDoc.getElementById("container");
  // console.log(qrDiv);

  let i = 0;
  arrSelectedAssets.forEach((asset) => {
    // loop through array
    arrPromises.push(
      // add async function promise to array
      generateQRFromURL(
        qrElement,
        "http://" +
          HOST_URL +
          "/html/Requests/request_create_guest.html?location=" +
          plant_name +
          "&asset=" +
          asset.text
      )
  ...
  Promise.allSettled(arrPromises).then((results) => {
    // wait for all promises and generate QRs
    ...
    downloadHTML(
      "qrcode.html",
      new Blob([new XMLSerializer().serializeToString(qrDoc)], {
        type: "text/html",
      })
    );
...
function startQRGeneration() {
  ...
  const arrSelecetedAssets = Array.from(
  ...
  if (arrSelecetedAssets.length > 40) {
    swal({
      title: "Are you sure?",
      text:
        arrSelecetedAssets.length +
        " QR codes are going to be generated. This might take a while",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    }).then((result) => {
