  ...
  let activity_log = [
    {
      ...
      activity: `Created ${tableInfo[req.body.type].name} Master ${
        tableInfo[req.body.type].name
      } `,
    },
  ];
...
const fetchMasterTypeSingle = async (req, res, next) => {
  ...
  if (!type || !id) return res.status(400).json();

  if (!(type in tableInfo)) return res.status(400).json("type does not exist");
...
const updateMasterTypeSingle = async (req, res, next) => {
  ...
  const activity_log = {
    ...
    name: req.user.name,
    role: req.user.role_name,
    activity: `Updated ${tableInfo[req.params.type].name} Master ${
  ...
  function verifyColumns(
    ...
    if (tableName == "plant_master") tableName = "plant";
    if (tableName == "system_master") tableName = "system";
    if (!(tableName in tableInfo)) return false;
    ...
    if (
      !tableInfo[tableName].fields.every(
        (x) =>
          columns.includes(x.column_name) &&
          entries[x.column_name] !== undefined
      )
    )
      return false;
  ...
  if (req.params.id === undefined) return res.status(404).json("no id");
